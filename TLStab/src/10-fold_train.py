...
def main(args):
    ...
    idx_dict_train = get_immuno_dataset("./Nested_CVs_stab/folds/" + str(fold) + "/train_" + str(fold) + ".csv")
    idx_dict_val = get_immuno_dataset("./Nested_CVs_stab/folds/" + str(fold) + "/val_" + str(fold) + ".csv")
    idx_dict_test = get_immuno_dataset("./Nested_CVs_stab/folds/test_" + str(fold) + ".csv")
    ...
    test_peptide_list = [x[0] for x in list(idx_dict_test.values())]

    train_size = len(train_peptide_list)
    valid_size = len(valid_peptide_list)
    test_size = len(test_peptide_list)
    ...
    test_loader = get_immuno_dataloader(idx_dict_test,
                                        {key: feats_dict[key] for key in test_peptide_list},
                                        batch_size=1, shuffle=False)


    del train_peptide_list
    del valid_peptide_list
    del test_peptide_list
    del feats_dict
    del idx_dict_train
    del idx_dict_val
    del idx_dict_test
    ...
    if model_type in ["finetune", "freeze", "new_layer"]: #Simply initialize weights
        model_path = "./Updated_Binding_Affinity_data_v2/models/torch_models/Go_to_BA_EL_model_UPDATED.pt"
    ...
    for epoch in range(epochs):
        ...
        with open('/home/rf27/Stab_models_paper_revision_v3/' + str(fold) + '/logs/' + str(effort) + '.txt', 'a') as f:
        ...
        #Testing section 
        test_loss, labels_list, preds_list = forwarding(model, test_loader, optimizer, test_size, action='valid', effort=effort, frozen = model_type)
        print('Test Average loss: {:.4f}'.format(test_loss))
        with open('/home/rf27/Stab_models_paper_revision_v3/' + str(fold) + '/logs/' + str(effort) + '.txt', 'a') as f:
            f.write('Test Average loss: {:.4f}\n'.format(test_loss))
        ...
        if best_val_loss == None or val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), '/home/rf27/Stab_models_paper_revision_v3/' + str(fold) + '/torch_models/' + str(effort) + '.pt')
